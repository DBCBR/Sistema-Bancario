...
def saque(clientes, nome, valor):
    ...
    if clientes[nome]['saques'] >= 3:
        return "Limite de saques excedido. Você só pode realizar 3 saques por acesso."
    ...
    if valor > clientes[nome]['saldo']:
        ...
        return f"Saque realizado com sucesso. Novo saldo: {locale.currency(clientes[nome]['saldo'], grouping=True)}"
...
def deposito(clientes, nome, valor):
    ...
    c.salvar_clientes(clientes)  # Salvar dados dos clientes
    return f"Depósito realizado com sucesso. Novo saldo: {locale.currency(clientes[nome]['saldo'], grouping=True)}"

def extrato(clientes, nome):
    historico = clientes[nome].get('historico', [])
    extrato = f"Nome: {nome}\nSaldo: {locale.currency(clientes[nome]['saldo'], grouping=True)}\nHistórico de transações:\n"
    for transacao in historico:
        extrato += f"{transacao['data']} - {transacao['tipo']}: {locale.currency(transacao['valor'], grouping=True)}\n"
    return extrato
